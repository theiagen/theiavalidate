...
class Validator:
  ...
  def __init__(self, options):
    ...
    self.logger.debug("The following options were passed to the Parser class:\n%s" % options)
  ...
  def convert_table_to_dataframe(self, table):
    self.logger.debug("Converting %s to a Pandas Dataframe" % table)
  ...
  and prints out the resulting table
  """
  ...
  """
  This function counts the number of populated cells in each table
  """  
  ...
  def percent_difference(self, value1, value2):
    ...
    # |x-y|/((x+y)/2)
    return np.absolute(value2.sub(value1)).div(value2.add(value1)/2)
  ...
  def validate(self, column):
    if column.name in self.table1.columns:
      ...
      if pd.api.types.is_string_dtype(column) == True: # if a string
        ...
        number_of_differences = self.percent_diffence(differences).sum()
