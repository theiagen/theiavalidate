...
from theiavalidate.CheckInputs import to_numeric_safe
...
class TestCompareFiles(unittest.TestCase):
    ...
    def test_matching_files_exact_matches(self):
        ...
        pd.testing.assert_frame_equal(
            self.validator.file_exact_matches, expected)
    ...
    def test_null_files_number_of_differences(self):
        ...
        pd.testing.assert_frame_equal(
            self.validator.file_number_of_differences, expected)
    ...
    def test_matching_files_exact_differences(self):
        ...
        expected.index = self.SAMPLES_INDEX
        pd.testing.assert_frame_equal(
            self.validator.file_exact_differences, expected)
...
class TestValidateFiles(unittest.TestCase):
    ...
    TABLE1_FILE_URIS = ["gs://match1-1.txt", "gs://mismatch1-2.txt",
                        "gs://match1-3", "gs://sortmatch1-1.txt", np.nan]
    TABLE2_FILE_URIS = ["gs://match1-1.txt", "gs://mismatch1-2.txt",
                        np.nan, "gs://sortmatch1-1.txt", np.nan]
    ...
    def setUp(self):
        ...
        self.validator.validation_criteria = (self.validator.validation_criteria
                                              .apply(to_numeric_safe).convert_dtypes()
                                              )
    ...
    def test_validation_table(self):
        ...
        self.validator.validation_table.set_index(
            self.validator.table1["samples"], inplace=True)
        self.validator.validation_table.rename_axis(
            None, axis="index", inplace=True)
        ...
        expected.columns = pd.MultiIndex.from_tuples(
            expected.columns, names=["Column", "Table"])
        pd.testing.assert_frame_equal(
            self.validator.validation_table, expected)
